...
## Nom du(des) étudiant(e)(s) de ce binôme
WARNING: NE PAS OUBLIER DE MENTIONNER LES DEUX NOMS SI VOUS ETES EN BINOME!

#Merat Melissa# et #Merlaud Elyne#
...
Le code n'est pas découpé en couches.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
La règle de gestion se trouve dans le fichier TodoListControl

*Cette règle est-elle facilement testable par un test unitaire ?*
Cette règle n'est pas facilement testable par un test unitaire car la méthde est privée
...
on créer et lance un conteneur docker en mode interactif 
l'option e permet de configurer une variable d'environnement, dans notre cas le mot de passe de la base
l'option p affiche tous les ports utilisés
On donne ensuite le maping du port

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`
On ajoute un nouvel item dans le repertoire

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
Le repository est null car il n'est pas initialisé.
Il faut mettre en place de l'injection de dépendance, en utilisant insomnia on a pu tester notre code.

* Modifier le code en conséquence.

* Tester vos endpoints avec un client REST.
Nous avons reçu un retour 201, la donnée a bien été ajouté à la base.
...
La donnée est bien visualisable dans les données de DBeaver.
...
On devre mettre en place des test d'intégrations

*Que teste-on dans ce cas ?*
On teste le bon fonctionnement du logiciel ou d'une partie précise de celui ci.
